# Sentiment_API — Scope + Tech Spec v1.1 (PRD‑style)
...
- Run event studies vs SPY/ES and store outcomes to improve calibration without overfitting (forward-only).

### G6 — Asset targeting (markets/sectors/tickers)
- For each high-signal cluster and rolling windows, rank:
  - most affected market (SP500 vs NASDAQ_COMPOSITE)
  - sector impacts (11 sectors)
  - ticker winners/losers (S&P 500 + Nasdaq Composite universes)
...
- maintain `sp500` and `nasdaq_composite` universes
- security master (ticker → sector → universe)
- endpoints: `/v1/universes/*`, `/v1/sectors`
...
### M5.5 — Asset Targeting Engine
- outputs: most affected market, sector impacts, ticker winners/losers
- endpoints: `/v1/impacts/latest`, `/v1/impacts/clusters/{cluster_id}`
...
- NFR2: p95 list endpoints < 300ms (impacts endpoints < 800ms default window)
...
See `SENTIMENT_API_MASTER_SPEC_v1.1.md` Section 17 (AC‑M0..AC‑M10).

---

## 11) Deliverables (v1.1)

- Postgres schema + migrations based on `db/schema.sql` (incl. security master)
