...
export default function OverviewPage() {
  ...
  const gaugeValue = useMemo(() => {
    const s = mood.data?.data?.sentiment;
    if (!s) return 0;
    if (s === "RiskOn") return 0.6;
    if (s === "RiskOff") return -0.6;
    if (s === "Neutral") return 0.0;
    return 0.0;
  }, [mood.data?.data?.sentiment]);

  const lineData = useMemo(() => {
    return (intraday.data?.data ?? []).map((p: any) => ({
      t: p.ts.slice(11,16),
      idx: p.index_value,
  ...
  const bars = useMemo(() => {
    ...
    const pts = (intraday.data?.data ?? []).slice(-24);
    ...
    <Shell>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-12 gap-[var(--grid-gap)]">
        <Card className="xl:col-span-4" title="Sentiment Indicator" subtitle="Health at first sight." right={
          <Badge tone={mood.data?.data?.sentiment === "RiskOff" ? "bad" : mood.data?.data?.sentiment === "RiskOn" ? "good" : "neutral"}>
            {mood.data?.data?.sentiment ?? "—"}
          </Badge>
        }>
          {!mood.data ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <Gauge value={gaugeValue} label="Mood" sublabel={`confidence ${fmtPct(mood.data.data.confidence ?? 0)}`} />
              <div className="space-y-2">
                <div className="glass p-3">
                  <div className="text-xs opacity-70">Intraday Index</div>
                  <div className="text-xl font-semibold">{(mood.data.data.index_intraday ?? 0).toFixed(2)}</div>
                  <div className="text-xs opacity-70">vol {mood.data.data.news_volatility_intraday?.toFixed(2)} • volume {mood.data.data.news_volume_intraday}</div>
                </div>
                <div className="glass p-3">
                  <div className="text-xs opacity-70">Risk Vector</div>
                  <div className="text-xs opacity-75">
                    geo {(mood.data.data.risk_vector?.geopolitical_risk ?? 0).toFixed(2)} • energy {(mood.data.data.risk_vector?.energy_supply_risk ?? 0).toFixed(2)}
                  </div>
                </div>
              </div>
            </div>
        ...
        <Card className="xl:col-span-3" title="Market Impacts" subtitle="Where the shock concentrates.">
          {!markets.data ? <Skeleton className="h-[210px]" /> : (
            <div className="space-y-2 text-sm">
              ...
              <div className="text-xs opacity-60 mt-2">method {markets.data.data.methodology_version}</div>
            </div>
          )}
        </Card>
      </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-12 gap-[var(--grid-gap)]">
        ...
        <Card className="xl:col-span-4" title="Sector Sentiment Performance" subtitle="Ranked heat (top).">
          {!sectors.data ? <Skeleton className="h-[180px]" /> : (
            <Table>
              ...
              <TBody>
                {(sectors.data.data.sectors ?? []).slice(0, 8).map((s: any) => (
                  <TR key={s.sector_id}>
        ...
        <Card className="xl:col-span-4" title="Sector Heatmap" subtitle="At-a-glance concentration.">
          {!sectors.data ? <Skeleton className="h-[180px]" /> : <SectorHeatmap sectors={sectors.data.data.sectors} />}
        </Card>
      </div>
        ...
        <Card className="xl:col-span-5" title="Top Gainers & Losers" subtitle="Ticker impacts (latest).">
          {!tickers.data ? <Skeleton className="h-[220px]" /> : (
            <div className="grid grid-cols-2 gap-3 text-sm">
              ...
              <div className="col-span-2 text-xs opacity-60 mt-2">method {tickers.data.data.methodology_version}</div>
            </div>
          )}
        </Card>
      </div>
        ...
        <Card className="xl:col-span-12" title="Latest High-Impact Clusters" subtitle="Evidence-first, drilldown on click.">
          {!clusters.data ? <Skeleton className="h-[180px]" /> : (
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-3">
              {(clusters.data.data ?? []).slice(0, 6).map((c: any) => (
