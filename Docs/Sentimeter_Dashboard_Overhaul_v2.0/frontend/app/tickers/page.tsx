    ...
    <Shell>
      ...
      {!tickers.data ? (
        ...
        <div className="grid grid-cols-1 xl:grid-cols-2 gap-[var(--grid-gap)]">
          <Card title="Winners" subtitle={`method ${tickers.data.data.methodology_version}`}>
            <Table>
              <THead>
                <TR hover={false}>
                  <TH>Ticker</TH>
              ...
              <TBody>
                {tickers.data.data.winners.slice(0, 25).map((t: any) => (
                  <TR key={t.symbol}>
          ...
          <Card title="Losers" subtitle="Highest negative impact">
            <Table>
              ...
              <TBody>
                {tickers.data.data.losers.slice(0, 25).map((t: any) => (
                  <TR key={t.symbol}>
                    <TD className="font-medium">{t.symbol}</TD>
                    <TD className="text-right">{Math.round(t.expected_return_bps)}</TD>
                    <TD className="text-right opacity-80">{Math.round(t.confidence * 100)}%</TD>
